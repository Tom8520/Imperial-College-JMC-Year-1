...
data TestCase = forall a b . (Reformat a, Show b, Eq b)
              ...
              | FileTestCase String ((String, String) -> String) [((String, String), String)]
...
goTest (FileTestCase name f cases) = do
  counts <- forM cases (runCase f)
  let passes = filter id counts
  ...
  where
    runCase f ((a, b), c) = do
      input1 <- readFile (testDir++a)
      input2 <- readFile (testDir++b)
      expected <- readFile (testDir++c)
      result <- evaluate (f (input1, input2))
