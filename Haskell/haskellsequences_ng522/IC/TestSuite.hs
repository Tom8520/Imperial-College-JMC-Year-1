...
data TestCaseWithError = forall a b . (Reformat a, Show b, Ord b, Num b, Fractional b)
              => TestCaseWithError String (a -> b) [(a, b)]

goTestWithError (TestCaseWithError name f cases) = do
  counts <- forM cases (handle majorExceptionHandler . goTestOneWithError name f)
  let passes = filter id counts
  putStrLn $ name ++ ": " ++ show (length passes)
                  ++ " / " ++ show (length counts)
  putStrLn ""
...
goTestOneWithError name f (input, expected) = handle exceptionHandler $ do
  r <- evaluate (f input)
